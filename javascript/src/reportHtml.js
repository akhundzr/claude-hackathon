...
th:hover{color:var(--text)}td{padding:.45rem .75rem;border-bottom:1px solid var(--border)}
...
.bw{background:rgba(255,152,0,.2);color:var(--yellow)}.be{background:rgba(244,67,54,.2);color:var(--red)}
  ...
  <div class="header-inner">
    <div>
      <svg class="gauge-svg" viewBox="0 0 200 115">
        <path d="M 20 100 A 80 80 0 0 0 180 100" fill="none" stroke="var(--accent)" stroke-width="12" stroke-linecap="round"/>
        <path id="gauge-fill" d="M 20 100 A 80 80 0 0 0 180 100" fill="none" stroke="#e94560" stroke-width="12" stroke-linecap="round" stroke-dasharray="251.33 502" stroke-dashoffset="251.33"/>
        <text id="gauge-grade" x="100" y="88" text-anchor="middle" font-size="44" font-weight="700" fill="var(--text)" font-family="system-ui,sans-serif">-</text>
        <text id="gauge-score-text" x="100" y="112" text-anchor="middle" font-size="12" fill="var(--text-dim)" font-family="system-ui,sans-serif">Score: -</text>
      </svg>
    </div>
...
<div class="container">
  <div class="card animate__animated animate__fadeInUp" style="animation-delay:.1s"><h2>Quality Score Breakdown</h2><div id="quality-bars"></div></div>
  <div class="card animate__animated animate__fadeInUp" style="animation-delay:.2s"><h2>Language Distribution</h2><div class="lang-chart-wrap"><div class="lang-chart-container"><canvas id="lang-chart"></canvas></div><div class="lang-legend" id="lang-legend"></div></div></div>
  <div class="card animate__animated animate__fadeInUp" style="animation-delay:.3s"><h2>Metrics Summary</h2><div class="table-wrap"><table id="metrics-table"><thead><tr><th data-col="lang">Language &#x25B2;</th><th data-col="files">Files</th><th data-col="loc">LOC</th><th data-col="comments">Comments</th><th data-col="blanks">Blanks</th><th data-col="functions">Functions</th><th data-col="classes">Classes</th></tr></thead><tbody id="metrics-body"></tbody></table></div></div>
  <div class="card animate__animated animate__fadeInUp" style="animation-delay:.4s"><div class="panel-header"><h2>Code Smells</h2><span class="badge bw" id="smell-w">0</span><span class="badge be" id="smell-e">0</span></div><div id="smells-content"></div></div>
  <div class="card animate__animated animate__fadeInUp" style="animation-delay:.5s"><div class="panel-header"><h2>Security Findings</h2><span class="badge be" id="sec-badge">0</span></div><div id="security-content"></div></div>
  <div class="card animate__animated animate__fadeInUp" style="animation-delay:.6s"><h2>File Explorer</h2><div class="file-toolbar"><input type="search" id="file-search" placeholder="Search files\u2026"></div><div class="table-wrap"><table><thead><tr><th data-fcol="path">File &#x25B2;</th><th data-fcol="language">Language</th><th data-fcol="loc">LOC</th><th data-fcol="functions">Fns</th><th data-fcol="smells">Smells</th><th data-fcol="security_issues">Security</th></tr></thead><tbody id="files-body"></tbody></table></div><div class="pagination" id="pagination"></div></div>
</div>
...
document.getElementById('s-files').textContent=R.summary.total_files.toLocaleString();document.getElementById('s-loc').textContent=R.summary.total_loc.toLocaleString();document.getElementById('s-dur').textContent=R.scan_metadata.scan_duration_ms.toLocaleString();document.getElementById('s-langs').textContent=Object.keys(R.summary.languages).length;
