...
export const FUNCTION_PATTERNS = {
  ...
  Go:         [/^\s*func\s+/],
  Rust:       [/^\s*fn\s+\w+\s*[(<]/],
  JavaScript: [/^\s*function\s+\w+\s*\(/, /^\s*const\s+\w+\s*=\s*(function\b|\([^)]*\)\s*=>|\w+\s*=>)/, /^\s*async\s+function\s+\w+\s*\(/, /^\s*export\s+(default\s+)?(async\s+)?function\s+\w+\s*\(/, /^\s*export\s+const\s+\w+\s*=\s*(function\b|\([^)]*\)\s*=>|\w+\s*=>)/],
  TypeScript: [/^\s*function\s+\w+\s*\(/, /^\s*const\s+\w+\s*=\s*(function\b|\([^)]*\)\s*=>|\w+\s*=>)/, /^\s*async\s+function\s+\w+\s*\(/, /^\s*export\s+(default\s+)?(async\s+)?function\s+\w+\s*\(/, /^\s*export\s+const\s+\w+\s*=\s*(function\b|\([^)]*\)\s*=>|\w+\s*=>)/],
